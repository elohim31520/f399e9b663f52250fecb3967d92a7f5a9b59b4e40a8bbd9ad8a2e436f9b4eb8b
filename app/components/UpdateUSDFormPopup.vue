		...
		<div class="p-2">
			...
			<van-form ref="formRef" @submit="onSubmit">
				<van-cell-group class="!rounded-xl overflow-hidden">
					<van-field v-model="form.balance" type="number" name="balance"
						:label="$t('update_usd_form_popup.usd_balance_label')"
						:placeholder="$t('update_usd_form_popup.usd_balance_placeholder')" :rules="[
							{
								required: true,
				...
				<div class="px-2 py-5">
					<van-button round block type="primary" color="#F88379" native-type="submit">
						{{ isUpdateMode ? $t('update_usd_form_popup.update_button') :
							$t('update_usd_form_popup.add_button') }}
					</van-button>
				</div>
			</van-form>
		</div>
...
const onSubmit = async () => {
	...
	try {
		...
		if (isUpdateMode.value) {
			await $bffApi.updateMyBalances(payload)
		} else {
			await $bffApi.createMyBalances(payload)
		}
		showToast({
			type: 'success',
